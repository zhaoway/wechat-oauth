...
function isBadAccessTokenErrcode( errcode )
{
    switch( errcode )
    {
        case 40001:
...
function isBadAccessTokenError( error )
{
    if ( error instanceof Error  &&  error.name == 'WeChatAPIError' )
    {
        return isBadAccessTokenErrcode( error.code );
    }
...
AccessToken.prototype.isValid = function () {
  return !!this.data.access_token && (new Date().getTime()) < (this.data.create_at + (this.data.expires_in -  60) * 1000);
};
 ...
 * @param {Function} clearToken 用于清除无效token的方法
 */
var OAuth = function (appid, appsecret, getToken, saveToken, clearToken) {
  this.appid = appid;
  ...
  this.clearToken = clearToken || function (openid, callback) {
    var token = this.store[openid];
...
OAuth.prototype.getUser = function (options, callback) {
  ...
  this.getToken(options.openid, function (err, data) {
    ...
    if (token.isValid()) {
      that._getUser(options, token.data.access_token, function ( err, result ) {
          
        if (err) {
          if ( isBadAccessTokenError( err ) )
          {
              that.clearToken( options.openid, function ( error, ret_token ) {  
            
                  if ( error )
                    console.log("clearToken error: " + error );
                  else
                    console.log("clearToken: " + ret_token );
...
/**
   return token of openid if it is valid after verify token.
  
   refresh if token timeout.
   
 **/
OAuth.prototype.verifyUserToken  = function (openid, callback) {
  ...
  this.getToken(openid, function (err, data) {
    ...
    if (token.isValid()) {
      ...
      that._verifyToken(openid, token.data.access_token, function (err, result) {

        if (err) {

          that.clearToken(openid, function (error, ret_token) {
